...
4. **Tree flatness** — pass-through wrapper components; props drilled through 2+ intermediates; trees deeper than page → section → primitive without reason; data that takes 3+ file jumps to trace — should be composition (`children`/slots) or data read at the point of use; props typed with an inline object type instead of a declared `<Component>Props` interface (local helpers included); props accepting a whole entity to read a couple of fields instead of declaring only what's consumed or deriving with `Pick<>`.
5. **Exports, files & naming** — default exports outside Next.js file conventions; barrel `index.ts` files; a file that imports a symbol only to re-export or alias it (the `'use client'` boundary file is the one exception); a consumer importing a symbol from a file other than the one that defines it; non-kebab-case filenames; `.tsx`/`.ts` mismatch with JSX presence; `I`-prefixed interfaces; boolean/handler/callback naming (`is/has/should`, `handle*`, `on*`).
6. **TypeScript** — inline type literals on parameters, state, or returns instead of a declared `interface`/`type`; `any` instead of `unknown`; restated types that should derive (`z.infer`, `keyof typeof`, `ReturnType`); enums where `as const` maps fit; invalid states made representable (parallel optional fields that should be a discriminated union). Leave pure-syntax preferences (`T[]` vs `Array<T>`, quote style) to the linter — don't spend review signal on them.
...
10. **Simplicity, traceability & scope** — nested conditionals where guard clauses fit; speculative abstraction; a helper, wrapper, or generic with a single caller; a type, constant, or helper moved to `types/`/`constants/`/`lib/` while only one file uses it (one concept scattered over several files); a shared function grown a flag or options bag to serve a second caller (should be two functions); logic that takes 3+ file jumps to follow; a clever construct where the plain one works; comments that restate the code; unrequested flexibility; changes untraceable to the task.
