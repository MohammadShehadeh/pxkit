...
- One public *concept* per file — a component together with its compound parts (`Card`, `CardHeader`, `CardContent` from `card.tsx`), or a hook; the filename mirrors the main export (`input-field.tsx` → `InputField`, `use-countdown.ts` → `useCountdown`). The file also holds everything private to that concept — sub-components, props interfaces, constants, helpers. "One per file" limits what the file is *about*, not how many declarations it has (see [components.md](components.md)).
- `.tsx` if and only if the file contains JSX. Hooks are logic-only `.ts`; anything returning JSX is a component in `components/` (and wildcard subpath exports resolve one extension per directory, see [structure.md](structure.md)).
